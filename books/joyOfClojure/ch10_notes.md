...
Vars have dynamic scope. Unlike lexical scope which is based on the nested structure of the code, dynamic scope allows vars to be initialized anywhere in the call stack. This can make it difficult to see where and how a var is getting a particular value. You run into some of the same sorts of problems that you get with global variables. Problems arise most often when you return a function that uses a var and then try to call it outside of the dynamic scope that it was created in. One might expect the function to close over the var's value the way it would with a lexically scoped local, but it won't. The value of the var a the time of dereferencing will be based on its current dynamic scope.
